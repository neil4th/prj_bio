...
function insertFingerprintDataIntoDatabase(fingerprintData) {
    // Convert the fingerprint data to a blob
    var blob = dataURItoBlob(fingerprintData, 'image/png');

    // Create a database connection
    var db = new mysqli('localhost', 'root', '', 'bio_s');

    // Insert the blob into the database
    var query = 'INSERT INTO bio_s (bios, name) VALUES (?, ?, ?)';
    db.query(query, [blob, 'Fingerprint Data']);
  }
  ...
  $("#save").on("click", function () {
    if (
        ...
        if (vDiv.children.length < 5) {
            ...
            // Send the message "Hello World" instead of the image to the API
            const message = "Hello World";  // Your custom message

            // Send data (message) to API using FormData
            ...
            fetch("http://localhost:8000/api/upload", {
                ...
                .then((data) => {
                    // Alert the server response
                    alert("Server Response: " + JSON.stringify(data));
                })
                .catch((error) => {
                    console.error("Error:", error);
                    alert("Error -> Failed to upload data: " + error.message);
                });
        } else {
